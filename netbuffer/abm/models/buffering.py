...
def buffer_parcels(settings, buffer_parcels_spec,
    ...
    """
    Performs network buffering (using Pandana libary
    http://udst.github.io/pandana/) for each point in parcel
    file using expressions from buffer_parcels_spec.

    The actual results depend on the expressions in
    buffer_parcels_spec, but this is initially intended to
    replicate PSRC's Soundcast (Daysim) parcel accessibilities.

    Using a point file representing the centroid of a land use
    boundary (parcel), quanitifes the amount of some variable
    within a specified distance or the distance to a variable.
    For example, the total number of jobs within a half mile of
    each parcel or the distance to the nearest bus stop from each
    parcel.
    ...
    network_fname = os.path.join(
        data_dir, buffer_parcels_settings["buffer_network"])
    ...
    parcel_data_df[constants['nodes-id']] = network.get_node_ids(
        parcel_data_df[constants['parcels-x']].values,
        parcel_data_df[constants['parcels-y']].values)
    parcel_data_df.set_index(
        constants['parcel_index'], inplace=True, drop=False)
    ...
    if 'trace_parcels' in buffer_parcels_settings:
        ...
        trace_parcel_rows = parcel_data_df[
            constants['parcel_index']].isin(trace_parcels)
    else:
        trace_parcel_rows = None

    # point of interest file (e.g. location of bus stops)
    poi_fname = os.path.join(
        data_dir, buffer_parcels_settings["transit_stops"])
    poi_df = pd.read_csv(poi_fname, index_col=False)
    poi_df[constants['nodes-id']] = network.get_node_ids(
        poi_df[constants["stops-x"]].values,
        poi_df[constants["stops-y"]].values)

    # intersections:
    # combine from and to columns
    all_nodes = pd.DataFrame(
        network.edges_df[constants["links-a"]].append(
    ...
    intersections_df = pd.DataFrame(
        all_nodes[constants['nodes-id']].value_counts())
    intersections_df = intersections_df.rename(
        columns={constants['nodes-id']: 'edge_count'})
    intersections_df.reset_index(0, inplace=True)
    intersections_df = intersections_df.rename(
        columns={'index': constants['nodes-id']})

    # add a column for each way count
    intersections_df['nodes1'] = np.where(
        intersections_df['edge_count'] == 1, 1, 0)
    intersections_df['nodes3'] = np.where(
        intersections_df['edge_count'] == 3, 1, 0)
    intersections_df['nodes4'] = np.where(
        intersections_df['edge_count'] > 3, 1, 0)
    ...
    results, trace_results, trace_assigned_locals \
        = buffer.buffer_variables(
            buffer_parcels_spec, 'parcels_df', locals_d,
            trace_rows=trace_parcel_rows)

    results = results.fillna(0)

    # since parcels_data_df is local in buffer.py,
    # it can come back with extra columns:
    del_cols = list(
        set(parcel_data_df.columns) - set(
            parcel_data.to_frame().columns))
    ...
    if trace_parcels:
        ...
        if trace_results is None:
            logger.warn(
                "trace_parcels not found in parcels = %s" % (
                    trace_parcels))
        else:
            df = parcel_data_df.loc[parcel_data_df[trace_parcel_rows].index]
            df = df.merge(
                trace_results, how='left', left_index=True, right_index=True)
    ...
    # write result
    buffered_parcels_fname = os.path.join(
        output_dir, buffer_parcels_settings["parcel_output_file"])
    pipeline.get_table(
        "parcel_data").to_csv(buffered_parcels_fname, index=False)
...
def create_network(settings, create_network_settings, data_dir):
    ...
    nodes = pd.read_csv(os.path.join(
        data_dir, create_network_settings["nodes"]))
    links = pd.read_csv(os.path.join(
        data_dir, create_network_settings["links"]))
    nodes.index = nodes[create_network_settings["nodes-id"]]

    network = pdna.Network(
        nodes[create_network_settings["nodes-x"]],
    ...
    network.save_hdf5(
        os.path.join(data_dir, create_network_settings["output"]))
